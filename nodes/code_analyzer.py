...
from prompts import ANALYZER_SYSTEM_PROMPT, ANALYZER_BACKGROUND
...
def code_analyzer(state: CodeReviewState) -> dict:
    """Review code for quality, structure, and best practices — prep, validate, and call LLM."""
    ...
    cached_prefix = {
        ...
        "background_knowledge": ANALYZER_BACKGROUND,
        "tool_definitions": ANALYZER_TOOLS,
    }
    user_message = f"Analyze this code for quality, structure, and best practices:\n\n{code}"
    ...
    try:
        ...
        logger.error("Code analyzer API call failed: %s", e)
        return {"findings": [{"type": "quality", "issue": f"API error: {e}", "severity": "medium", "source": "code_analyzer", "input_tokens": 0, "cache_tokens": 0}]}
    ...
    logger.info("Code analysis complete — input_tokens=%d, cache_tokens=%d", usage.input_tokens, getattr(usage, "cache_read_input_tokens", 0) or 0)
