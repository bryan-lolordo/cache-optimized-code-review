...
from prompts import PERFORMANCE_SYSTEM_PROMPT, PERFORMANCE_BACKGROUND
...
def performance_optimizer(state: CodeReviewState) -> dict:
    """Review code for performance bottlenecks — prep, validate, and call LLM."""
    ...
    cached_prefix = {
        ...
        "background_knowledge": PERFORMANCE_BACKGROUND,
        "tool_definitions": PERFORMANCE_TOOLS,
    }
    user_message = f"Analyze this code for performance bottlenecks and optimization opportunities:\n\n{code}"
    ...
    try:
        ...
        logger.error("Performance optimizer API call failed: %s", e)
        return {"findings": [{"type": "performance", "issue": f"API error: {e}", "severity": "medium", "source": "performance_optimizer", "input_tokens": 0, "cache_tokens": 0}]}
    ...
    logger.info("Performance review complete — input_tokens=%d, cache_tokens=%d", usage.input_tokens, getattr(usage, "cache_read_input_tokens", 0) or 0)
